/**
 * Schema del database, applicato in automatico alla prima query.
 *
 * Specchio di db/migrations/001_create_rsvp.sql (che resta la fonte per
 * `npm run db:migrate`): il deploy su Vercel non include db/ e scripts/
 * (vedi .vercelignore), quindi l'app non può dipendere da loro per
 * trovarsi le tabelle. Tutto è idempotente (if not exists / or replace):
 * rieseguirlo a ogni avvio a freddo non fa danni.
 *
 * Se cambi una migration, aggiorna anche questo file: lib/db-schema.test.ts
 * controlla che i nomi di tabelle e indici coincidano.
 */
